...
Model::Model(QString file_name,QString file_name_mtl) : CollisionEngine(file_name)
{
...
/*void Model::Draw(QGLShaderProgram * shaderProgram, QMatrix4x4 pMatrix,QMatrix4x4 vMatrix, QMatrix4x4 mMatrix, QVector3D LightPosition)
{
    ...
    shaderProgram->setAttributeArray("vertex", vertices.constData());       //"przesyłamy" wierzchołki obiektu
    shaderProgram->enableAttributeArray("vertex");

    shaderProgram->setAttributeArray("color", color.constData());           //"przesyłamy" kolory wierzchołków obiektu
    shaderProgram->enableAttributeArray("color");

    shaderProgram->setAttributeArray("normal", normal.constData());         //"przesyłamy" wektory normalne obiektu
    shaderProgram->enableAttributeArray("normal");

    glDrawArrays(GL_TRIANGLES, 0, vertices.size());         //rysujemy obiekt
    shaderProgram->disableAttributeArray("vertex");             //wyłączamy atrybuty
...
void Model::DrawLifeBar(QGLShaderProgram * shaderProgram, QMatrix4x4 pMatrix,QMatrix4x4 vMatrix, QMatrix4x4 mMatrix, QVector3D LightPosition,float life)
{
    shaderProgram->bind();
    shaderProgram->setUniformValue("mvpMatrix", pMatrix * vMatrix * mMatrix);       //ustawiamy wartości zmiennych uniform do shadera
    shaderProgram->setUniformValue("mvMatrix", vMatrix * mMatrix);
    shaderProgram->setUniformValue("lifeColor",life);
    buffer->bind();
    int offset = 0;
    shaderProgram->setAttributeBuffer("vertex", GL_FLOAT, offset, 3, 0);
    shaderProgram->enableAttributeArray("vertex");
